...
| 跨客户端设置转换、复制与初始化 | 移除 | 三端使用本端设置及默认值；旧文档结构升级仅保留存储兼容，历史迁移诊断不参与订阅校验 |
...
| sing-box 字段宽松透传 | 增加固定版本校验 | 官方 1.15.0-alpha.8 Schema 检查字段，图检查负责实际引用与循环 |
...
Surge、Clash 和 sing-box 不再互相转换、复制或初始化客户端设置。全新安装或从版本 1 配置升级时，使用本端原生 DNS、TUN 入站、出口接口检测、Proxy 策略组和 FINAL 规则默认值，Tailscale 连接留空；已保存的各端配置保持原样。跨端迁移模块、`SingboxConfig.migrationIssues` 字段及诊断处理面板已移除，配置规范化在读取、导入和保存时丢弃全部历史 `migrationIssues`，订阅生成不再读取这些记录。当前配置的 Schema、引用和规则校验继续生效。版本 1 / 2 到版本 3 的文档结构升级保留，Clash 本端原生规则转规则编排也保留。
...
按仓库规则使用类型检查、Worker 构建、公开文件扫描、浏览器实际操作与本地 API 请求，不新增或修改测试代码。官方 sing-box 1.15.0-alpha.8 检查完整输出与编译规则源。GitHub 发布、远程 Secrets/KV 修改及线上部署均不属于本次实现验证。
