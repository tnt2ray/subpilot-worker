...
On a new installation, setup creates local `wrangler.jsonc` and creates or reuses the `SUBPILOT_CONFIG` KV namespace. R2 is optional and is not created or bound by default. Setup asks for the source refresh interval (1–24 hours, default 12), checks remote Secrets, requests an admin token of at least 24 characters when the admin credential is missing, and uses a supplied or generated encryption key when that Secret is missing. It deploys with the missing Secrets through a temporary file that is removed afterward. Keep the admin token in a password manager.

Standard Worker (JSON) and GitHub Actions do not need R2. To enable WASM later, run `npm run setup -- --enable-r2` from the deployment project directory; setup creates a private bucket, adds the `RULE_SET_ARTIFACTS` binding and redeploys. A saved WASM preference stays selected and resumes automatically. If `wrangler.jsonc` already exists, setup reuses it, verifies and preserves existing Secrets, and writes only missing ones; an existing explicit R2 binding is reused. After an invalid token or failed deployment interrupts first-time setup, correct the problem and rerun `npm run setup`; an existing configuration file does not skip unfinished Secret initialization. Setup stops without writing Secrets if their remote state cannot be verified. `npm run setup -- --force-secrets` also deploys and replaces both Secrets; use it only for an intentional reset or planned rotation. **Preserve the existing `CONFIG_ENCRYPTION_KEY` when reusing encrypted KV data.**
...
| `SUBPILOT_RULE_SET_ARTIFACTS_BUCKET` | Optional R2 bucket name; setting it enables R2, and R2 is not created when omitted |
...
2. The following command creates a remote KV namespace; copy its returned `id` into `kv_namespaces[0].id` in `wrangler.jsonc`:

   ```bash
   wrangler kv namespace create SUBPILOT_CONFIG
   ```

3. Set your Worker name in `wrangler.jsonc`. R2 is not required for Standard Worker. To enable WASM, run `npm run setup -- --enable-r2` after preparing the base deployment; set `SUBPILOT_RULE_SET_ARTIFACTS_BUCKET` first to choose a custom bucket name.

4. Choose an admin token of at least 24 characters and calculate its SHA-256 hex value. This Bash command prompts without echoing the token:
...
5. These commands write remote Worker Secrets. Supply the hash above for `ADMIN_TOKEN_HASH` and a long random string for `CONFIG_ENCRYPTION_KEY`; retain an existing encryption key when reusing KV data:
...
6. The following command deploys to the Worker configured in `wrangler.jsonc`:
...
- sing-box generates a separate domain set: `-dns.json` with Standard Worker or temporary fallback, `-dns.srs` when WASM artifacts are ready, or `Sing-Box/<rule-set name>/dns-domains.srs` when Actions artifacts are ready, containing standalone DOMAIN, DOMAIN-SUFFIX, DOMAIN-KEYWORD, DOMAIN-REGEX and DOMAIN-WILDCARD rules, excluding IP, process and logical rules. Bindings follow routing-list order before native DNS-tab rules; empty domain sets produce a notice. Update bindings after deleting or renaming a referenced DNS server before saving.
...
Clash and Surge sources require conversion for sing-box even with one URL. `IP-ASN` expands into IPv4/IPv6 CIDRs and updates periodically. Resolution failures use cached data when possible; otherwise the ASN is skipped with a notice. Unsupported rules such as `USER-AGENT` and `URL-REGEX` are skipped. Download failures and invalid source formats are reported as errors. Worker-generated remote rule sets download through the `Proxy` group (`http_client.detour: "Proxy"`), including Actions SRS, Worker-hosted JSON / SRS and native SRS sources. Select a working proxy in this group in your client. This affects rule downloads only, not the traffic policies of the rules.
...
Generated files use `.list` for Surge and `.yaml` for Clash. sing-box uses `.json` with the default Standard Worker mode or temporary fallback, and `.srs` when WASM or Actions artifacts are ready. The same name can be used independently across clients.

### Rule compilation modes

Choose one preferred mode under **System settings → Rule compilation → Preferred compilation mode** and save. Standard Worker is the default. If WASM or Actions fails or its artifacts are pending, Standard Worker provides temporary fallback. Ready preferred artifacts are used automatically, without changing the saved mode. This choice affects rules requiring merging or conversion; direct client downloads and native SRS sources retain their existing behavior.

| Mode | Processing location | sing-box artifacts | Surge / Clash artifacts |
| --- | --- | --- | --- |
| Standard Worker (default) | Worker fetches, merges, deduplicates, converts and buckets rules without calling WASM | JSON | `.list` / `.yaml` |
| WASM | Worker fetches sources; the shared WASM kernel merges, deduplicates, rebuckets and compiles SRS | SRS when ready; JSON during fallback | `.list` / `.yaml` |
| GitHub Actions | GitHub fetches sources and runs the identical WASM kernel | SRS when ready; JSON during fallback | `.list` / `.yaml` |

WASM and Actions use the same pinned WASM module for merging, deduplication and rebucketing, with SRS compilation based on sing-box **1.15.0-alpha.7**. Both modes share source parsing and conversion, the aggregation kernel and the SRS compiler path. WASM SRS artifacts are encrypted in R2; Actions artifacts are published to GitHub. R2 and Actions are independent, optional features: Standard Worker and Actions do not depend on R2. If WASM is selected without R2 configured, the saved preference remains WASM while Standard Worker provides temporary fallback. System settings show how to enable it. Actions no longer downloads a separate native sing-box CLI. When preferred compilation fails, artifacts are pending or the current Actions refresh is unconfirmed, the Worker reads or prepares separate fallback caches, producing JSON for sing-box. This temporary fallback also applies when artifacts from a previous run still exist. Fallback does not change the selected mode or overwrite WASM or Actions artifacts. Preferred compilation continues retrying; once ready, its artifacts are used on the next client subscription configuration update, returning sing-box to SRS. Worker fallback caches can also be pending, and source retrieval or conversion can fail; requests may still return HTTP `503` with `Retry-After` or an appropriate error. Retry later or correct the rules as indicated. Switching preferred modes preserves the repository, branch and encrypted credentials. Actions configuration and controls appear only when Actions is selected.

Without R2, Standard Worker stores encrypted text and JSON artifacts in KV. With R2 configured, Standard Worker can store artifacts there; WASM SRS artifacts require R2. Separate Worker fallback caches stay in KV and never overwrite preferred artifacts. R2 manifests and the durable cleanup queue use KV; replacing or deleting a rule set cleans up the corresponding old R2 objects, with the five-minute maintenance task retrying when R2 is temporarily unavailable. New installs and upgrades do not create R2 by default. To enable WASM, run `npm run setup -- --enable-r2`, or set `SUBPILOT_RULE_SET_ARTIFACTS_BUCKET` before setup.

After changing preferred modes, update client subscription configurations to use the corresponding rule format and URLs. Switching between fallback JSON and restored SRS also takes effect on the next subscription configuration update; refreshing rule files alone does not change their declared format. Previously issued managed `.json` URLs keep returning JSON, even after preferred WASM / Actions recovers, with the Worker preparing compatible JSON when needed. They never return SRS binary content. Old managed `.srs` URLs serve usable WASM binaries or redirect to the corresponding published SRS in Actions mode. Without valid SRS, they request a subscription configuration update instead of returning JSON.

### Actions rule compilation setup
...
3. Enable and save a rule plan for at least one client. Open **System settings → Rule compilation**, select **GitHub Actions → Setup wizard** and enter the repository, workflow callback address and token.
   Use the setup wizard both to configure and replace the token. An existing token is shown as a mask; keep it unchanged to reuse it, or enter a new value to replace it. On desktop, labels and inputs share aligned columns; narrow screens stack them. Related links follow their fields, and the branch input in Advanced settings uses the same alignment. The token field shows only the repository selection reminder, without repeating the permission list. “Create token (preset permissions)” preselects no expiration (subject to organization lifetime policies), Actions, Contents and Secrets read/write plus Workflows write; choose the resource owner and limit access to the target repository on GitHub. A question mark beside the outer Rule compilation heading combines compilation, public rule visibility, token permissions and setup guidance; the wizard title has no question mark. System settings show Actions configuration and controls only when Actions mode is selected. Telegram hides Chat ID and binding controls until a Bot Token is entered. Hiding fields preserves their values.
4. Select **Check, install and enable**. The wizard installs `compile-rule-sets.yml`, the runner and the shared compiler, saves the new settings and submits the initial batch. On success, the page immediately updates the mode and repository details without saving other drafts. Credentials are encrypted separately; GitHub Secrets store `SUBPILOT_ACTIONS_SECRET` and `SUBPILOT_URL`.
...
6. Open **View compilation progress** to inspect Surge, Clash and sing-box separately. Use temporary Worker fallback rules until the current compilation is confirmed. Once preferred artifacts are confirmed, the next subscription configuration update automatically returns to Actions, switching sing-box from JSON to SRS. GitHub accepting a request does not prove it is running; check repository Actions for queue and execution details.

When first selecting Actions, after rule-plan changes, or after compilation failures, missing confirmed artifacts trigger temporary Standard Worker fallback while background work submits or waits for Actions jobs. If any required preferred artifact is missing for a client, that client uses Standard Worker fallback rules throughout; all required fallback artifacts must also be ready. Compilation progress shows fallback readiness per rule set, which does not establish readiness of the complete subscription. Confirmed publications remain usable during refresh failures when their plan is unchanged. Subscription updates wait or report errors only when required preferred artifacts and fallback rules are both unavailable, or sources or configuration contain errors; fallback does not guarantee that every request succeeds immediately.

Saved rule changes, manual refresh and daily refresh can trigger Actions. One batch covers all clients and downloads each source once. Published outputs are reused when source bodies, compilation inputs and valid ASN data are unchanged. When recompilation is needed, the runner downloads the same WASM kernel as the Worker from this deployment’s authenticated endpoint once per batch, verifies the SHA-256 pinned in its installed script, and reuses it for all clients’ rule aggregation and sing-box SRS compilation. An output failure does not stop other outputs. Existing five-minute maintenance checks recover unfinished jobs with a 60-minute automatic dispatch interval; **Resubmit compilation** bypasses that interval. No additional GitHub schedule is needed.
...
The preferred mode is stored in `settings.ruleCompilationMode` as `worker`, `wasm` or `actions`, defaulting to `worker`; temporary fallback never changes this field. Existing configurations without a mode migrate to `actions` when `actionsCompilation.enabled` is `true`. `settings.actionsCompilation` retains the repository and branch, with `enabled` always synchronized to `mode === "actions"`; status is available at `GET /api/actions-compilation/status`. The Worker fixes the workflow filename to `compile-rule-sets.yml`. The page no longer exposes a filename setting, and installation and dispatch ignore the legacy client-supplied field. A one-time upgrade removes the old filename from the active KV configuration and retires its obsolete protocol records, reusing existing migration state and stopping scans after completion. Existing repository details, branch, enabled state, encrypted token, shared secret and callback address remain available. Current settings and credentials take precedence; cleared credentials are not restored. Deployments using another workflow filename must run the setup wizard to install the fixed filename; Worker fallback rules can be used while Actions artifacts are unavailable. When Actions is enabled but the workflow has not been updated, the first admin-page visit automatically attempts installation once using the saved repository, callback address and token, showing progress and results without asking for confirmation. If it fails, users can correct the settings and retry. Skipping saves Standard Worker mode, which uses JSON rules for sing-box. Successful setup or switching to another mode stops the prompt without extra KV reminder markers.

Legacy migration is a one-time upgrade task. Only unfinished work uses the existing five-minute schedule to persist renamed settings and migrate encrypted credentials and the callback address in bounded batches. After read-back verification and a propagation grace period of at least five minutes, it removes old SRS credentials, addresses, protocol markers, job caches and publication receipts. Once complete, it reads the existing completion state and returns immediately, without scanning, decrypting or cleaning legacy data. Interrupted work resumes from saved progress. Rule contents used by Worker modes and the usual three configuration rollback versions remain available; no manual KV operations are required.
...
Encrypted rule-plan snapshots remain in KV for 24 hours for authenticated Actions downloads. Normal Actions runs download original source bodies to temporary runner storage, remove them on completion and exclude them from the repository. Standard Worker, WASM and temporary fallback use encrypted local source and compiled-rule caches. Worker fallback caches for Actions are independent of its remote artifacts. The Worker keeps Actions publication metadata without storing or proxying its artifact bodies. Rule-set names and generated rules are public; source addresses, Worker addresses and credentials are excluded from artifacts and logs. The token and shared secret are encrypted separately and excluded from configuration exports. Replacing the token preserves the shared secret; clearing and reconfiguring requires reinstalling the workflow. `ADMIN_TOKEN_HASH` and `CONFIG_ENCRYPTION_KEY` remain Worker Secrets.

Deployment builds run `npm run build:worker`: this verifies the pinned WASM kernel and checksum, copies the identical file into static assets for authenticated download, and bundles the shared compiler for the Actions setup wizard. The built WASM module and checksum ship with the source and are included in deployment artifacts, but Standard Worker mode never calls the module; rebuilding them requires Go 1.25.5 or newer, while running and deploying do not need Go. The Actions script is generated under ignored `dist/`, and the static kernel copy under ignored `public/vendor/rule-kernel.wasm`; neither generated output is committed. Dependency installation builds them as well. The wizard still installs only the workflow and two JavaScript files, without adding the WASM binary to GitHub. Update the workflow and scripts through the setup wizard after the Actions protocol upgrade.
...
Read the [release notes](https://github.com/tnt2ray/subpilot-worker/releases) first. Preserve the Worker name, KV ID, custom routes, and existing Secrets in local `wrangler.jsonc`. **The following command updates local program files, installs runtime dependencies, and deploys to that Worker:**

```bash
npm run update
```

For a Git clone, it requires clean tracked files and pulls the current branch with `--ff-only`. For a release-archive installation, it prefers the latest `subpilot-worker-vX.Y.Z.tar.gz`, falls back to the source archive when that attachment is absent, and replaces managed program files. Both paths preserve your Worker configuration and existing Secrets. The setup step reuses an existing `RULE_SET_ARTIFACTS` binding; if an older config has no binding, it leaves R2 disabled, which does not affect Standard Worker or Actions. To enable WASM later, run `npm run setup -- --enable-r2` from the project directory; it creates a private bucket, configures the binding and redeploys. The script also preserves the existing subscription schedule and adds the five-minute pending-work schedule. Do not remove `wrangler.jsonc` or replace `CONFIG_ENCRYPTION_KEY`: existing encrypted data depends on the same key.

`npm run update -- --no-deploy` still updates code, dependencies, and local configuration, preserving an existing R2 choice; it skips only the final Worker deployment, so it is not a read-only check.
...
Rules prefer artifacts matching the selected mode, current configuration and compiler version. Standard Worker uses JSON. WASM / Actions uses SRS when ready, otherwise separate Worker JSON fallback caches while preferred artifacts are prepared in the background. First use, rule changes or an upgrade can make both preferred and fallback artifacts unavailable, producing HTTP `503` with `Retry-After` while background batches prepare rules. Source or format errors are reported with their actual cause. **Subscription check** also starts preparation and reports its status. When preferred artifacts recover, the next client subscription configuration update automatically returns to SRS; previously issued JSON URLs still return JSON.

Saving configuration that affects rule output immediately records pending work in KV and starts a background update. If the execution budget runs out, the five-minute schedule continues unfinished work. Closing the admin page or stopping client updates does not stop this progress. When R2 is configured, the five-minute task also retries durable R2 cleanup records; it does not refetch all upstream sources every five minutes. Setup and update preserve an existing R2 binding but do not create one by default. Enabling R2 reuses `CONFIG_ENCRYPTION_KEY`; no Worker Secret is added.
...
The following describes local Standard Worker, WASM and temporary fallback caches. Actions retries, logs and remote artifact retention follow the Actions setup section above.

Failed background compilations briefly back off before retrying so other rule sets can make progress. Recognized source-format or configuration-reference errors appear in subsequent subscription checks and return HTTP `422` on subscription downloads. For Standard Worker, when only ASN data has expired and the configuration and compiler version still match, existing complete artifacts remain available while ASN data refreshes in the background. WASM / Actions temporarily uses Worker fallback rules until preferred artifacts recover. An incomplete background ASN lookup without usable cached prefixes prevents publishing an artifact with missing rules. Background work has an execution budget, so large batches may need several scheduled continuations to finish.

Rule sources share one complete encrypted body per URL; subscription sources share one per URL and effective User-Agent. Successfully fetched new content replaces earlier content without keeping source history. When R2 is available, preferred artifacts write a separate version before updating the current-version pointer and cleaning up old objects. Without R2, Standard Worker artifacts are stored in KV. Worker fallback artifacts are kept separately in KV. Edge cache entries use stable URLs with version validation and overwrite previous responses.

After deleting a rule set or source, or changing its URL and saving, background cleanup removes unreferenced source bodies, metadata and configured R2 compiled artifacts. Caches still used by another active rule or client remain. Source configuration entries with no rule-entry references are also removed. R2 deletion runs synchronously; failed operations remain in the KV cleanup queue for retry. Old edge copies expire according to their cache lifetime; rule-download endpoints stop using deleted entries once they read the newly saved configuration.
...
| One remote input | 4 MiB per subscription source; 16 MiB per text rule source downloaded by the Worker or Actions, reserving room for encryption and memory; client downloads are exempt |
...
| Standard Worker compilation | No fixed rule-count cap; source sizes, runtime resources and artifact storage capacity still apply |
| WASM / Actions structured aggregation | Input and output are each limited to 8 MiB; exceeding the limit uses standard Worker fallback rules without changing the preferred mode |
| One SRS artifact | Up to 24 MiB |
...
- Configuration snapshots, subscription/rule-source bodies, and recoverable subscription read tokens are stored encrypted in Workers KV. Worker compilation manifests, version indexes and durable cleanup records use KV metadata; the current-version pointer also has an encrypted R2 copy when R2 is enabled. Telegram tokens and other private configuration values are protected within the encrypted snapshot.
- Standard Worker Surge/Clash text and sing-box JSON are encrypted in KV by default, or in the private R2 bucket when configured. WASM SRS requires the private R2 bucket. Both use `CONFIG_ENCRYPTION_KEY`; preserve that key across updates and migration.
