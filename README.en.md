...
5. Use this deployment's workers.dev address for callbacks to avoid custom-domain bot challenges. Only address format is checked; no browser or Worker connectivity probe runs. The first run verifies connectivity and dispatch permission. Review completed steps and compilation progress on failure. Force retry bypasses the 60-minute interval for unfinished outputs with ready caches. `GET /api/singbox/srs/status` reports stages, not percentage progress.
