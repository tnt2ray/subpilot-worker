...
Settings use `settings.actionsCompilation`; status is available at `GET /api/actions-compilation/status`. Upgrades retain the repository, branch, custom workflow filename and enabled state saved in `settings.singboxSrs`. The old default `singbox-srs.yml` automatically becomes `compile-rule-sets.yml`; configuration reads, saves and wizard installation use the same name without adding a KV migration task. When current records are absent, the Worker reads the existing encrypted token, shared secret and callback address from KV, so they do not need to be entered again. Current settings and credential records always take precedence; cleared credentials are not restored. The old SRS workflow protocol and artifact paths are no longer used. Existing deployments must update the workflow through the setup wizard; the Worker supplies rules in the meantime, and obsolete artifacts can be removed separately.
