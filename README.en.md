...
This feature is disabled by default. When enabled, the Worker continues to merge, deduplicate and convert text sources using the current rule plan, then sends them to GitHub Actions for compilation with sing-box **1.15.0-alpha.6** and publication to a dedicated branch (default `srs`) in a public GitHub repository. Each output uses `format: "binary"` and fixed repository `.srs` URLs only after all its current routing and DNS artifacts are confirmed. Outputs awaiting confirmation automatically use `format: "source"` and Worker-hosted JSON URLs. User-supplied native `.srs` URLs continue to be downloaded directly by the client.
...
2. Create one fine-grained GitHub token scoped to the target repository with **Actions, Contents, Workflows and Secrets: Read and write**. The same token handles installation and ongoing compilation. Complete organization approval if required.
3. Open **System settings → sing-box SRS compilation → Setup wizard**, enter the repository and callback address, and supply the token. Leave it blank to reuse an existing token with all these permissions. Reopening the wizard restores the callback address from the last successful installation. Older versions did not save it, so enter it once after upgrading. When managing through a custom domain, the wizard no longer substitutes the current admin origin as the callback address.
4. Choose **Check, install and enable**. The token is encrypted in independent KV storage; the shared secret is generated and synchronized to GitHub Secrets. The wizard automatically updates existing workflow and compiler files, saves SRS settings and starts initial compilation, preserving other drafts. Do not revoke the token while SRS is enabled.
5. Use this deployment's workers.dev address for callbacks to avoid custom-domain bot challenges. Only address format is checked; no browser or Worker connectivity probe runs. The first run verifies connectivity and dispatch permission. Review completed steps and compilation progress on failure. For unfinished outputs with ready caches, select **Resubmit compilation** to bypass the 60-minute interval. `GET /api/singbox/srs/status` reports stages, not percentage progress.

**View compilation progress** shows the number of ready rule sets, each rule set's stage and suggested action, and the last submission attempt time. **Awaiting compilation result** means GitHub accepted the request, but completion is not yet confirmed; check repository Actions for queue status, execution steps and failures. The page reflects the saved configuration; select **Refresh status** for the latest result. Once all rule sets are ready, update your sing-box subscription to use SRS.

One Action checks all SRS outputs in a batch, downloading the compiler once. Confirmed unchanged outputs are skipped. If the repository already contains the current job, only publication confirmation is retried. Per-output failures do not stop later outputs; one run summarizes the batch. Publication confirmation reads the receipt from the specified commit and validates its job and artifacts. On download failure, the Worker event `srs_receipt_download_failed` contains only the endpoint type, HTTP status or exception category, never repository URLs, receipt bodies or credentials. Upgrade the workflow and script through the setup wizard; mismatched workflow protocols prevent dispatch. Automatic batch retries wait 60 minutes; force retry queues another batch. HTTP `403` while downloading a task means the callback endpoint refused access. The script distinguishes explicit Cloudflare challenge responses and directs users to review the callback address and security settings without printing addresses or response bodies.

6. Use **Configuration links → Subscription check → sing-box** and update the client subscription. Complete JSON rule caches allow the full configuration to download during initial SRS compilation, after compilation failures, or while awaiting publication confirmation; the check reports JSON fallback as a warning. Update the subscription again to use each output's confirmed binaries. Only missing required JSON caches return HTTP `503` with `Retry-After` while preparation runs in the background.

Compilation uses the existing triggers: saving rule changes, manual refresh, and daily rule-source change detection. Complete artifacts are reused when source content and other compiler inputs have not changed. The existing five-minute maintenance task checks unfinished SRS work and may dispatch it again after at least 60 minutes since the previous attempt; no GitHub schedule is required. Enabling the feature or changing the repository, workflow ref, output branch or workflow also prepares SRS files for the current rules. Dispatch or compilation failures do not invalidate ready JSON caches or block JSON subscriptions; review compilation progress or Actions and retry. Ordinary deployments preserve completion records. Changes to rules, the compiler or artifact protocol can invalidate those records; JSON remains available while the new binaries are prepared. Disable the option and save to use JSON for all managed outputs.
...
The workflow downloads the job's source snapshot from the Worker and publishes all routing/DNS binaries and a receipt in one Git commit on the output branch, creating that branch if necessary. It uses the built-in `GITHUB_TOKEN` with `contents: write`; no additional write PAT is needed. The output branch must differ from the default and workflow branches, and branch protection must permit Actions to write it. Concurrent updates retry branch conflicts without force-pushing or replacing other rule sets. The Worker confirms the current version's public publication receipt before referencing that output's SRS files in newly generated configurations; JSON is used until then.

The output branch organizes artifacts by rule-set name under `rule-sets/`, for example `rule-sets/OpenAI/`. Chinese names are supported. Special characters, long names and reserved names are converted to safe names with a short digest to avoid collisions. Each folder has a `README.md` explaining its files; only files needed by the rule set are generated:

| File | Purpose |
| --- | --- |
| `routing.srs` | Routing rules |
| `domains.srs` | Domain routing rules |
| `ip-ranges.srs` | IP range routing rules |
| `dns-domains.srs` | Domain subset for DNS matching |
| `manifest.json` | Publication receipt; no manual configuration needed |

Publication, receipt validation and generated configurations all use `rule-sets/<name>/`. Stable URLs use `https://raw.githubusercontent.com/<owner>/<repo>/refs/heads/<output-branch>/rule-sets/<name>/<file>.srs`, with each path segment automatically URL-encoded. Updating a rule set under the same name preserves its URLs. Reading, copying, dual publishing and fallback logic for old hash directories have been removed. After upgrading the Worker, update the workflow and script through the setup wizard to regenerate current rules under the named directories. Refresh client subscriptions once all outputs are ready to switch to the new URLs. Old directories are no longer maintained; existing repository files and Git history are not automatically deleted.

GitHub caching can delay when clients see updates. The repository must be public, making rule-set names and compiled rule contents publicly readable; publish only rules suitable for public access. Subscription-source URLs, Worker URLs and credentials are not written to the output branch or Actions logs. Disabling the feature or deleting/renaming an output does not remove published files or Git history; clean up the repository separately when needed. Use a dedicated repository for each SubPilot deployment; Actions Secrets are shared within the repository.

Encrypted compilation source snapshots remain in KV for 24 hours. The Worker stores publication status and maintains its existing JSON source caches, but does not store or proxy SRS binaries. The persistent GitHub token and shared secret use a separate encrypted KV record, excluded from ordinary configuration responses, snapshots and exports. The callback address from the last successful installation is also encrypted separately and returned only to the administrator's setup wizard. The shared secret is synchronized to GitHub Actions Secrets; a token supplied solely to an installation request is not persisted by the installer.
...
sing-box configuration and hosted rule downloads only read complete JSON rule caches matching the current configuration and compiler version. When first use, rule changes, or an upgrade invalidate those JSON caches, the server promptly returns HTTP `503` with `Retry-After` and prepares rules in background batches. Retry the configuration update after the indicated delay. **Subscription check** also starts preparation and reports its status. A complete configuration is returned once every required JSON cache is ready. Optional SRS compilation does not block subscriptions: pending outputs use JSON, and confirmed binaries are selected on the next subscription update.
