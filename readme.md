...
此功能默认关闭。启用后，Worker 仍按现有规则计划合并、去重和转换文本来源，再通过 GitHub Actions 使用 sing-box **1.15.0-alpha.6** 编译为 SRS，并发布到公开 GitHub 仓库的独立产物分支（默认 `srs`）。每个规则集在当前版本的路由和 DNS 产物全部确认后，生成配置才使用 `format: "binary"` 与仓库文件的固定 `.srs` 地址；尚未就绪的规则集自动使用 `format: "source"` 与 Worker 托管的 JSON 地址。用户提供的原生 `.srs` 地址继续由客户端直接下载。

1. 准备你控制的已初始化公开 GitHub 仓库，并启用 GitHub Actions。一键安装使用仓库默认分支；产物使用独立分支。每个 SubPilot 部署使用独立仓库，避免仓库级 Actions Secrets 相互覆盖。
2. 创建一个 fine-grained GitHub Token，仅选择目标仓库，授予 **Actions、Contents、Workflows、Secrets: Read and write**。同一个 Token 用于安装和日常编译，组织仓库如需审批请先完成审批。
3. 打开 **系统设置 → sing-box SRS 编译 → 配置向导**，填写仓库和工作流访问地址，输入该 Token。已有 Token 可留空沿用，但必须具备上述全部权限。访问地址会记住上次成功安装时的值，重新打开向导时自动回填；旧版没有保存该地址，升级后需填写一次。通过自定义域名管理时，不会再自动把当前管理地址填作工作流回调地址。
4. 点击 **检查、安装并启用**。Token 加密保存到独立 KV；共享密钥自动生成并同步到 GitHub Secrets。向导自动更新已有工作流与编译脚本，保存 SRS 设置并启动首次编译；不保存其他页面草稿。启用期间不要撤销这个 Token。
5. 可填写本部署的 workers.dev 地址作为工作流访问地址，避免自定义域名的人机验证；仅检查格式，不进行浏览器或 Worker 连通性探测。首次运行验证实际连接及触发权限。失败时查看已完成步骤和编译进度，修正后重试。未完成且缓存已就绪的规则集可点击“重新提交编译”，跳过 60 分钟间隔。状态接口为 `GET /api/singbox/srs/status`，显示任务阶段而非百分比。

“查看编译进度”展示已就绪规则集数量、各规则集的处理阶段与操作提示，以及最近提交尝试的时间。等待编译结果表示 GitHub 已接收请求，尚未确认完成；具体排队情况、执行步骤和失败原因请查看仓库 Actions。页面显示已保存配置的状态，需点击“刷新状态”获取最新结果；全部就绪后，在 sing-box 中更新订阅即可使用 SRS。

一次 Action 批量检查全部 SRS，编译器只下载一次；已确认且未变化的规则集跳过。仓库已发布当前任务但回执未确认时，仅重试确认，不重新编译。单项错误不会阻止后续规则集处理，结束后汇总一次运行结果。发布确认从指定提交读取回执并核对任务与产物；下载失败时，Worker 日志的 `srs_receipt_download_failed` 只记录入口类型、HTTP 状态或异常类别，不记录仓库地址、回执正文或凭据。升级须通过配置向导更新仓库工作流与脚本；工作流协议不匹配时不会触发。批次自动重试间隔为 60 分钟，强制重试会再排入一个批次。下载任务时收到 HTTP `403` 表示回调入口拒绝了访问；脚本会区分明确的 Cloudflare 人机验证响应并提示检查工作流访问地址及防护设置，不输出地址或响应正文。

6. 执行 **配置链接 → 订阅检查 → sing-box** 并更新客户端订阅。只要 JSON 规则缓存完整可用，即使首次 SRS 编译、编译失败或等待发布确认，仍可正常下载完整配置；检查结果以警告提示 JSON 回退。各规则集的 SRS 就绪后，再次更新订阅即可使用对应的二进制产物。只有必需的 JSON 缓存本身缺失时，才返回 HTTP `503` 和 `Retry-After` 并在后台准备。

编译沿用保存规则变更、手动刷新和每日规则来源变化检测的触发方式；源内容及其他编译输入未变化且产物完整时，不重复编译。原有每 5 分钟待办续建任务检查未完成的 SRS 编译，距上次触发至少 60 分钟后可重新触发；无需新增 GitHub 定时任务。首次启用或更换仓库、工作流分支、产物分支、工作流后也会为当前规则补建 SRS。触发或编译失败不会把已生成的 JSON 缓存标记为失败，也不会阻断 JSON 订阅；可在编译进度或 Actions 查看原因并重试。普通部署不清除完成记录；规则、编译器或产物协议变化导致原记录不再适用时，暂用 JSON 并等待新版本完成。关闭开关并保存后，所有托管规则集使用 JSON。
...
工作流从 Worker 获取加密快照对应的规则正文，将路由与 DNS 产物以及发布清单作为一次 Git 提交写入产物分支；没有该分支时自动创建。工作流使用自带的 `GITHUB_TOKEN`（`contents: write`），无需额外的写仓库 PAT。产物分支不得为仓库默认分支或工作流所在分支，且分支保护规则需允许 Actions 写入。不同规则集并行发布时会重试冲突，不强制推送，也不覆盖其他规则集。Worker 确认当前版本的公开发布清单后才在新生成的配置中引用该规则集的 SRS，此前使用 JSON。

产物分支的 `rule-sets/` 目录按规则集名称组织，例如 `rule-sets/OpenAI/`。支持中文名称；特殊字符、过长名称和保留名称会转成安全名称并附加短摘要以避免重名。目录内的 `README.md` 说明每个文件的用途，只生成当前规则集需要的文件：

| 文件 | 用途 |
| --- | --- |
| `routing.srs` | 路由匹配规则 |
| `domains.srs` | 域名路由规则 |
| `ip-ranges.srs` | IP 网段路由规则 |
| `dns-domains.srs` | 用于 DNS 匹配的域名子集 |
| `manifest.json` | 发布校验信息，无需手动配置 |

产物发布、回执校验和生成配置统一使用 `rule-sets/<name>/`。固定地址格式为 `https://raw.githubusercontent.com/<owner>/<repo>/refs/heads/<output-branch>/rule-sets/<name>/<file>.srs`，路径各段自动进行 URL 编码；同名规则集更新内容不会改变地址。旧哈希目录的读取、复制、双写和回退逻辑已移除。升级 Worker 后，通过配置向导更新工作流和脚本，后台会在新目录重新生成当前规则；全部就绪后刷新客户端订阅以切换到新地址。旧目录不再维护，仓库中已有文件和 Git 历史不会自动删除。

GitHub 的缓存可能使客户端稍晚看到更新。仓库须公开，规则集名称和 SRS 规则内容均可被任何人读取；不要发布不适合公开的规则。订阅源地址、Worker 地址及访问凭据不会写入产物分支或 Actions 日志。关闭功能或删除、重命名规则集不会删除已经公开的文件或 Git 历史，需要时自行清理仓库。每个 SubPilot 部署应使用独立仓库，Actions Secrets 在仓库内共享。

编译源快照以加密形式在 KV 保留 24 小时，Worker 仅保留发布状态，不储存或代理 SRS 二进制；源 JSON 仍按原有缓存机制维护。长期 GitHub Token 和共享密钥存放在独立的加密 KV 记录中，不进入普通配置接口、配置快照或导出文件。上次成功安装的工作流访问地址也独立加密保存，仅在管理员配置向导中回填。共享密钥另同步至 GitHub Actions Secrets；单次请求额外提供的安装 Token 不会由安装接口保存。
...
sing-box 配置和托管规则文件下载只读取与当前配置及编译版本匹配的完整 JSON 规则缓存。首次生成、规则变更或升级导致 JSON 缓存失效时，服务端及时返回 HTTP `503` 和 `Retry-After`，并在后台分批生成规则；请按提示稍后再次更新配置。“订阅检查”也会触发准备并显示当前状态，全部必需 JSON 缓存就绪后即返回完整配置。可选的 SRS 编译不阻断订阅：未就绪时使用 JSON，就绪后在下次更新订阅时使用 SRS。
