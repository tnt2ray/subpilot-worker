...
5. 可填写本部署的 workers.dev 地址作为工作流访问地址，避免自定义域名的人机验证；仅检查格式，不进行浏览器或 Worker 连通性探测。首次运行验证实际连接及触发权限。失败时查看已完成步骤和编译进度，修正后重试。未完成且缓存已就绪的规则集可“强制重试”，跳过 60 分钟间隔。状态接口为 `GET /api/singbox/srs/status`，显示任务阶段而非百分比。
