...
| Clash | `behavior`：domain / ipcidr / classical；`interval` 为下载间隔，单位秒，默认 86400 |
| sing-box | 自动识别或指定 Clash、Surge、域名、IP-CIDR、classical 来源格式 |

单个兼容的 Surge / Clash 来源由客户端直接下载，Worker 不下载或缓存。同一条目包含多个 URL 时，系统合并去重；Clash 自动生成 `rule-providers`，无需手写。不同 Clash / sing-box 条目保持独立，不按出口跨条目聚合。Surge 可选“按策略聚合”，启用前请核对匹配顺序。
