...
路由页不再提供“规则来源”模式切换框，直接显示当前配置对应的编辑器。新配置使用原生规则；已有来源编排保留编辑、保存和生成能力，移除入口不改变其规则或输出顺序，无需 KV 数据迁移。编译模式的来源配置、选择、策略、顺序、内联规则和单条规则均独立保存。同一 URL 的来源正文缓存可复用，编译产物按客户端隔离。删除本端来源不会删除其他端仍使用的正文缓存。
