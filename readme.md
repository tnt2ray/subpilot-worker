...
sing-box 配置和托管规则文件下载只读取与当前配置及编译版本匹配的完整规则缓存。首次生成、规则变更或升级导致缓存失效时，服务端及时返回 HTTP `503` 和 `Retry-After`，并在后台分批生成规则；请按提示稍后再次更新配置。“订阅检查”也会触发准备并显示当前状态，全部必需规则就绪后才返回完整配置。
