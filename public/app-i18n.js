export const I18N = {
  ...
  surgeTailscaleHelp: "每个节点会生成一条 tailscale 策略和对应的 [Tailscale <section-name>] 配置段；需要 Surge iOS 5.20.0+ 或 Surge Mac 6.7.0+。",
  ...
  surgeTailscaleIdleKeepalive: "空闲保活（秒）",
  ...
  surgeTailscaleValidationError: "Tailscale 节点配置无效：名称必须完整且唯一，启用节点必须填写 Auth Key；测速 URL 只能使用有效的 http:// 地址；底层策略必须存在且最终可用，并且不能形成自引用或循环。",
  surgeRules: "Surge 规则",
  ports: "端口",
